...
public class ProductReviews extends AbstractResponse<ProductReviews> {
    ...
    public ProductReviews(JsonObject fields) throws SchemaViolationError {
        for (Map.Entry<String, JsonElement> field : fields.entrySet()) {
            String key = field.getKey();
            String fieldName = getFieldName(key);
            switch (fieldName) {
                case "items": {
                    List<ProductReview> list1 = new ArrayList<>();
                    for (JsonElement element1 : jsonAsArray(field.getValue(), key)) {
                        ProductReview optional2 = null;
                        if (!element1.isJsonNull()) {
                            optional2 = new ProductReview(jsonAsObject(element1, key));
                        }
    ...
    public List<ProductReview> getItems() {
        return (List<ProductReview>) get("items");
    }

    public ProductReviews setItems(List<ProductReview> arg) {
    ...
    public ProductReviews setPageInfo(SearchResultPageInfo arg) {
