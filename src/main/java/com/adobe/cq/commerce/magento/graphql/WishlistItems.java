...
public class WishlistItems extends AbstractResponse<WishlistItems> {
    ...
    public WishlistItems(JsonObject fields) throws SchemaViolationError {
        for (Map.Entry<String, JsonElement> field : fields.entrySet()) {
            String key = field.getKey();
            String fieldName = getFieldName(key);
            switch (fieldName) {
                case "items": {
                    List<WishlistItemInterface> list1 = new ArrayList<>();
                    for (JsonElement element1 : jsonAsArray(field.getValue(), key)) {
                        WishlistItemInterface optional2 = null;
                        if (!element1.isJsonNull()) {
                            optional2 = UnknownWishlistItemInterface.create(jsonAsObject(element1, key));
                        }
    ...
    public WishlistItems setItems(List<WishlistItemInterface> arg) {
    ...
    public WishlistItems setPageInfo(SearchResultPageInfo arg) {
