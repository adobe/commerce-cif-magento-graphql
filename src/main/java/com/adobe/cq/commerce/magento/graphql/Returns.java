...
public class Returns extends AbstractResponse<Returns> {
    ...
    public Returns(JsonObject fields) throws SchemaViolationError {
        for (Map.Entry<String, JsonElement> field : fields.entrySet()) {
            String key = field.getKey();
            String fieldName = getFieldName(key);
            switch (fieldName) {
                case "items": {
                    List<Return> optional1 = null;
                    if (!field.getValue().isJsonNull()) {
                        List<Return> list1 = new ArrayList<>();
                        for (JsonElement element1 : jsonAsArray(field.getValue(), key)) {
                            Return optional2 = null;
                            if (!element1.isJsonNull()) {
                                optional2 = new Return(jsonAsObject(element1, key));
                            }
    ...
    public Returns setItems(List<Return> arg) {
    ...
    public Returns setPageInfo(SearchResultPageInfo arg) {
    ...
    public Returns setTotalCount(Integer arg) {
